...
function SimTimeStep(dt, positions, velocities, springs, stiffness, damping, particleMass, gravity, restitution) {
    ...
    for (let i = 0; i < springs.length; i++) 
	{
		...
		//We take each element of the spring
	...
	//We are updating the velocity and the position of the particles.
    ...
    for (let i = 0; i < positions.length; i++) 
	{
    	...
    	for (let j of ['x', 'y', 'z']) 
		{
			...
			if (positions[i][j] < -1) 
			{
				//Compute hDistance to find the new position of the particle after a collision is detected
				let hDistance = Math.abs( positions[i][j] - (-1) );
				positions[i][j] += hDistance + ( restitution * hDistance ); 
				
				//We invert velocity with restitution applied
				velocities[i][j] *= -restitution; 
			} else if (positions[i][j] > 1) 
			{
				hDistance = Math.abs( positions[i][j] - (1) );
				positions[i][j] -= hDistance + ( restitution * hDistance );
